...
Orchestrates 62 real scanners. Preserves evidence. Correlates attack paths. Refuses to pretend a scan happened.
...
[![Tools](https://img.shields.io/badge/wired%20scanners-62%20of%2094-purple.svg)](#wired-scanners)
...
- **Runs real scanners** — 62 are wired end-to-end today across core engines, AppSec language packs, recon, fuzzers, K8s/cloud, IaC, API testing, parameter mining, JS endpoint + secret extraction, GraphQL auditing, secret scanning, XSS validation, YARA detection, and SARIF importers. The other ~32 are registered in the catalog with truthful state (`planned` / `reference` / `analyzer` / `importer` / `blocked`).
...
| Web parameter + JS mining (3) | arjun · linkfinder · secretfinder |
| Dependency / SCA (1) | retirejs |
| API testing (4) | schemathesis · jwt-tool · graphql-cop · kiterunner |
...
**62 wired, 94 in the catalog.** The remaining ~32 are visible under `/arsenal` with truthful state (`planned` / `reference` / `analyzer` / `importer` / `blocked`) — intentionally not runnable until their parsers land.
...
| 25 | **+5 wired scanners** — arjun (parameter mining), linkfinder + secretfinder (JS mining), graphql-cop (GraphQL audit), kiterunner (API discovery). 62 of 94 now runner-ready. |
